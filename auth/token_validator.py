...
class EntraTokenClaims:
    ...
    def groups(self) -> list[str]:
        # requires groupMembershipClaims=SecurityGroup in app manifest
        # >200 groups triggers overage — check has_groups_overage
    ...
    def is_app_only(self) -> bool:
        # app-only = Managed Identity / client credentials — no user in the token
...
class EntraTokenValidator:
    # test mode: EntraConfig.for_testing() + set_test_keypair() — no Azure needed
    ...
    def _validate_production(self, token: str) -> EntraTokenClaims:
        ...
        try:
            ...
            logger.info("JWKS kid mismatch, refreshing: %s", e)
            self._refresh_jwks()
            try:
                signing_key = self._jwks_client.get_signing_key_from_jwt(token)
            except PyJWKClientError as e2:
                raise EntraTokenValidationError(f"signing key not found: {e2}")
        ...
        try:
            ...
            raise EntraTokenValidationError("token expired")
        except jwt.InvalidAudienceError:
            raise EntraTokenValidationError(
                f"audience mismatch — expected {self._config.audience!r}"
            )
        except jwt.InvalidIssuerError:
            raise EntraTokenValidationError(
                f"untrusted issuer for tenant {self._config.tenant_id!r}"
            )
        except jwt.InvalidTokenError as e:
            raise EntraTokenValidationError(f"validation failed: {e}")

        # Validate tenant claim to prevent cross-tenant token use
        if claims.get("tid") != self._config.tenant_id:
            raise EntraTokenValidationError(f"tenant mismatch: got {claims.get('tid')!r}")
    ...
    def _refresh_jwks(self) -> None:
        ...
        try:
            ...
            raise EntraTokenValidationError(f"OIDC metadata fetch failed: {e}")
    ...
    def _validate_test(self, token: str) -> EntraTokenClaims:
        ...
        if self._test_public_key is None:
            raise EntraTokenValidationError("no test key set — call set_test_keypair() first")
        try:
            claims = jwt.decode(
                token,
            ...
            raise EntraTokenValidationError(f"test token invalid: {e}")
        return EntraTokenClaims(claims)
